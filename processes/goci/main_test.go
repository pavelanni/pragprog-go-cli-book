...
func TestRun(t *testing.T) {
	...
	var testCases = []struct {
		...
		{name: "fail", proj: "./testdata/toolErr",
			out:    "",
			expErr: &stepErr{step: "go build"}, setupGit: false, mockCmd: nil},
		{name: "failFormat", proj: "./testdata/toolFmtErr",
			out:    "",
			expErr: &stepErr{step: "go fmt"}, setupGit: false, mockCmd: nil},
		{name: "failTimeout", proj: "./testdata/tool",
			out:    "",
			expErr: context.DeadlineExceeded, setupGit: false, mockCmd: mockCmdTimeout},
	}
...
func setupGit(t *testing.T, proj string) func() {
	...
	var gitCmdList = []struct {
		...
		{[]string{"remote", "add", "testrepo", remoteURI}, projPath, nil},
...
func TestHelperProcess(t *testing.T) {
	...
	if os.Args[2] == "git" {
		fmt.Fprintln(os.Stdout, "Everything is up-to-date")
		os.Exit(0)
	}
	os.Exit(1)
}
